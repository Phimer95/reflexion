...
def generic_generate_internal_tests(
    ...
    if model.is_chat:
        if is_react:
            ...
            output = model.generate_chat(messages=messages, max_tokens=1024, temperature=temperature)
...
def generic_generate_internal_test_refinement(
        func_sig: str,
        flawed_test: str,
        feedback: str,
        model: ModelBase,
    ...
    """Generates a refined version of a unit test."""
    messages = [
        Message(
            role="system",
            content=test_generation_chat_instruction,
