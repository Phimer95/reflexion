...
from .generator_utils import generic_generate_func_impl, generic_generate_internal_tests, generic_generate_self_reflection, generic_generate_internal_test_refinement
...
PY_REFINED_TEST_GENERATION_CHAT_INSTRUCTION = """You are an AI coding assistant that can refine a unit test for a function given the signature, the flawed unit test and the reflection for this unit test."""
PY_REFINED_TEST_FEW_SHOT = """
[func signature]: def encrypt(s: str) -> str: 'Create a function encrypt that takes a string as an argument and returns a string encrypted with the alphabet being rotated. The alphabet should be rotated in a manner such that the letters shift down by two multiplied to two places. For example: >>> encrypt('hi') 'lm' >>> encrypt('asdfghjkl') 'ewhjklnop' >>> encrypt('gf') 'kj' >>> encrypt('et') 'ix' '
[flawed test]: assert encrypt('ABCDEFGHIJKLMNOPQRSTUVWXYZ') == 'CDEFGHIJKLMNOPQRSTUVWXYZAB'
[feedback]: The function instruction specifies that the function encrypt should take a string as an argument and return a string encrypted with the alphabet being rotated. The rotation should be such that the letters shift down by two multiplied to two places. This means each letter in the input string should be replaced by the letter that is four places ahead of it in the alphabet.
    The provided unit test is checking the function with the input string 'abcdefghijklmnopqrstuvwxyz' and expects the output to be 'cdefghijklmnopqrstuvwxyzab'. However, this is not correct based on the function instruction. The expected output is not shifted four places down the alphabet, but only two.
    For example, the first letter 'a' should be replaced by 'e' (four places down the alphabet), but in the expected output, it is replaced by 'c' (two places down the alphabet). The same applies to all other letters in the string.
    Therefore, the unit test is not logically correct. The correct test should be: "assert encrypt('abcdefghijklmnopqrstuvwxyz') == 'efghijklmnopqrstuvwxyzabcd'"
[refined test]: assert encrypt('ABCDEFGHIJKLMNOPQRSTUVWXYZ') == 'EFGHIJKLMNOPQRSTUVWXYZABCD'
"""
...
class PyGenerator(Generator):
    ...
    def internal_tests(self, func_sig: str, model: ModelBase, max_num_tests: int = 5, temperature: float = 0.2) -> List[str]:
    ...
    def generate_refined_test(self, func_sig: str, flawed_test: str, feedback: str, model: ModelBase) -> List[str]:
        """
        Generates tests for a function.
        """
        return generic_generate_internal_test_refinement(
            func_sig=func_sig,
            flawed_test=flawed_test,
            feedback=feedback,
            model=model,
            test_generation_few_shot=PY_REFINED_TEST_FEW_SHOT,
            test_generation_chat_instruction=PY_REFINED_TEST_GENERATION_CHAT_INSTRUCTION,
            is_syntax_valid=py_is_syntax_valid,
        )
